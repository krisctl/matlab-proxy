...
function App() {
  ...
  if (dialogModel) {
    ...
    switch (dialogModel.type) {
      case "confirmation":
        ...
        dialog = (
          <Confirmation confirm={confirm} cancel={closeHandler}>
            {dialogModel.message || ""}
          </Confirmation>
        );
        break;
  ...
  if (isConnectionError) {
    dialog = (
      <Error message="Either this integration terminated or the session ended">
        <p>
          Attempt to <a href="../">return to a parent app</a>
        </p>
      </Error>
    );
  ...
  // Give precendence to token auth over licensing info ie. once after token auth is done, show licensing if not provided.
    ...
    !licensingProvided &&
    hasFetchedServerStatus &&
    (!authEnabled || isAuthenticated)
  ) {
    overlayContent = (
      <LicensingGatherer role="licensing" aria-describedby="license-window" />
    );
  }
  ...
  const overlay = overlayVisible ? <Overlay>{overlayContent}</Overlay> : null;
